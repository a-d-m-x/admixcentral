<x-app-layout>
    <x-slot name="header">
        <x-firewall-header title="{{ __('Edit VXLAN Interface') }}" :firewall="$firewall" />
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                <x-card-header title="Edit VXLAN Interface" />
                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('interfaces.vxlans.update', [$firewall, $id]) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl">
                            ...
                            <div>
                                ...
                                <x-text-input id="deviceId" class="block mt-1 w-full" type="number" min="0" max="65535"
                                    name="deviceId" :value="old('deviceId', $vxlan['deviceId'] ?? '')" required autofocus />
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                    Numeric device identifier (e.g. 1 for <code>vxlan1</code>).
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="vxlanid" class="block mt-1 w-full" type="number" min="1" max="16777215"
                                    name="vxlanid" :value="old('vxlanid', $vxlan['vxlanid'] ?? '')" required />
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <div>
                                ...
                                <x-text-input id="vxlanlocal" class="block mt-1 w-full" type="text"
                                    name="vxlanlocal" :value="old('vxlanlocal', $vxlan['vxlanlocal'] ?? '')" required placeholder="e.g. 192.168.240.11" />
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <div>
                                ...
                                <x-text-input id="vxlanlocalport" class="block mt-1 w-full" type="number" min="1" max="65535"
                                    name="vxlanlocalport" :value="old('vxlanlocalport', $vxlan['vxlanlocalport'] ?? '')" placeholder="Default: 4789" />
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="vxlanremote" class="block mt-1 w-full" type="text"
                                    name="vxlanremote" :value="old('vxlanremote', $vxlan['vxlanremote'] ?? '')" placeholder="e.g. 192.168.240.12" />
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <div>
                                ...
                                <x-text-input id="vxlanremoteport" class="block mt-1 w-full" type="number" min="1" max="65535"
                                    name="vxlanremoteport" :value="old('vxlanremoteport', $vxlan['vxlanremoteport'] ?? '')" placeholder="Default: 4789" />
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="vxlangroup" class="block mt-1 w-full" type="text"
                                    name="vxlangroup" :value="old('vxlangroup', $vxlan['vxlangroup'] ?? '')" placeholder="e.g. 239.0.0.1" />
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <div>
                                @php
                                    $curDev = is_array($vxlan['vxlandev'] ?? null) 
                                        ? (collect($vxlan['vxlandev'])->filter(fn($v) => !empty($v['selected']))->keys()->first() ?? '')
                                        : ($vxlan['vxlandev'] ?? '');
                                @endphp
                                ...
                                <select id="vxlandev" name="vxlandev"
                                    ...
                                    @foreach($interfaces as $ifCode => $ifData)
                                        <option value="{{ $ifCode }}" {{ old('vxlandev', $curDev) === $ifCode ? 'selected' : '' }}>
